...
async def change_forms_quantity(message: types.Message, state: FSMContext):
    ...
    if str(message.from_id) not in admins:
        ...
        set_config_parameter("SMARTAGENT", "forms_quantity", message.text)
    ...
    await message.answer("Количество форм должно состоять только из цифр")
    return