...
async def change_trials_finds_form_quantity(message: types.Message, state: FSMContext):
    ...
    if str(message.from_id) not in admins:
        ...
        set_config_parameter("SMARTAGENT", "trials_finds_form_quantity", message.text)
        await message.answer(create_config_data_message()[1], reply_markup = admin_board, parse_mode = "HTML")
        await BotStates.admin_menu_state.set()
        return
    ...
    await message.answer("Количество пробных форм должно состоять только из цифр")
    return